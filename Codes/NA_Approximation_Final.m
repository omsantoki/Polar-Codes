...
K = 128;
N = 512;
EbN0_dB_range = -2:0.25:2;
EbN0_range = 10.^(EbN0_dB_range/10); 

P_NA = zeros(size(EbN0_range));
...
for i = 1:length(EbN0_range)
    ...
    argument = sqrt(N/V) * (C - r + (log2(N))/(2*N));
    P_NA(i) = qfunc(argument);
end

EbN0_min = (2^(r) - 1)/(r);
EbN0_min_dB = 10*log10(EbN0_min);

disp('Eb/N0 (dB) | Normal Approximation P_e');
disp('------------------------------------');
for i = 1:length(EbN0_dB_range)
    fprintf('%6.1f     | %12.4e\n', EbN0_dB_range(i), P_NA(i));
end
fprintf('\nShannon Limit for rate %.2f: %.4f dB\n', r, EbN0_min_dB);

figure;
semilogy(EbN0_dB_range, P_NA, 'b--', 'LineWidth', 2); hold on;
xline(EbN0_min_dB, 'k:', 'LineWidth', 2);
grid on;
xlabel('E_b/N_0 (dB)');
ylabel('Block Error Probability (P_{N,e})');
title(sprintf('Reference Performance (r = %.2f, N = %d)', r, N));
ylim([10e-4 10e0]);
xlim([-2 2]);
...
semilogy(EbNodB_all,BER_all,'-o', 'LineWidth', 2, 'MarkerSize', 4, 'MarkerFaceColor', 'r');
